...
export function mapMerakiWebhookAlert(raw: MerakiWebhookAlertPayload): IAlertCisco {
  ...
  const deviceName: string = raw.deviceName ?? alertData.deviceName ?? alertData.name ?? "";
  const deviceMac: string = raw.deviceMac ?? alertData.deviceMac ?? alertData.mac ?? "";
  const deviceSerial: string =
    raw.deviceSerial ?? alertData.deviceSerial ?? alertData.serial ?? "";
  ...
  const productType: string =
    alertData.productType ?? alertData.deviceType ?? raw.deviceType ?? "";
  ...
  return {
    ...
    categoryType: raw.alertType ?? "",
    network: { id: raw.networkId ?? "", name: raw.networkName ?? "" },
    startedAt: raw.occurredAt ?? raw.sentAt ?? new Date().toISOString(),
    dismissedAt: null,
